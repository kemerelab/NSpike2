  ...
  : QWidget(parent)
{
    ...
    sendFileID = fileMenu->insertItem( "&Send File to User Program", this, SLOT(send())); 
...
void PulseFileTab::sendFileToUser(void)
{
    if (digioinfo.outputfd) {
	QString t = textEdit->text();
	const char *toUser = t.ascii();
	SendMessage(digioinfo.outputfd, DIO_PULSE_SEQ, (char *) toUser, 
		textEdit->length() * sizeof(char));
  fprintf(stderr,"spikeUserGUI - user program length = %d\n", textEdit->length());
