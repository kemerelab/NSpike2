/* spikeUserData.cpp: qt object for selecting data sent to spike_userdata
 *
...
void userDataDialog::setUserDataInfo() {
	...
	userdatainfo.sendpos = TRUE;
    }
    if (digioSelect->isChecked()) {
	userdatainfo.senddigio = TRUE;
    }
    userdatainfo.sendcont = FALSE;
    userdatainfo.sendspike = FALSE;
    for (i = 1; i <= sysinfo.maxelectnum; i++) {
	if (electSpikeSelect[i]->isChecked()) {
	    userdatainfo.spikeelect[i] = TRUE;
	    userdatainfo.sendspike = TRUE;
	}
	else {
