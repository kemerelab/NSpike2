  ...
  /* using the data type on each machine we set up the essential connections*/
{
  ...
  for (i = 0; i <= netinfo.nslaves; i++) {
    ...
    if ((sysinfo.datatype[i] & SPIKE) || (sysinfo.datatype[i] & CONTINUOUS)) {
      /* MESSAGE Connections */
      /* MAIN to DAQ */
      strcpy(netinfo.conn[c].from, netinfo.machinename[i]);
      netinfo.conn[c].fromid = SPIKE_MAIN;
	...
	if (sysinfo.dspinfo[j].machinenum == i) {
      ...
      /* if we need a dsp data connection, create it. Note that only one 
       * server is required, so we pretend that the data come from dsp 1  */
	...
	if (sysinfo.datatype[i] & USERDATA) {
	 ...
	 /* if digital IO is handled by a separate DSP, we need to
