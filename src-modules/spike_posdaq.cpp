    ...
    *bufferinfo = (time_sec - starttime.tv_sec) *
    ...
    YUY422to422P(outbuf, yuvbuf, yuvbuf+imagesize, yuvbuf+imagesize+(imagesize/2));                
		...
		if (sysinfo.acq) {
		     ...
		     * mass out, send it on to spike_main */
		    ...
		    /* send the data */
		    if (SendMessage(client_data + SPIKE_PROCESS_POSDATA, 
				POS_DATA, (char *) yuvbuf, imagesize * BYTES_PER_PIXEL) == -1) {
			error = 1;
		    } 
		    if (error) {
	...
	while ((id = netinfo.messageinfd[j]) != -1) {
		...
		switch(message) {
			...
			sysinfo.acq = 1;
#ifndef NO_VIDEO_DEBUG
			/* start the capture process and get the system
			 * time */
			start_capturing();
#endif
			...
			break;
		    case SYSTEM_INFO:
			/* read in the type of data, the buffersamp, the number of channels
			...
			/* currently the image size is fixed. It would be nice to fix this in
			 * a future version */
