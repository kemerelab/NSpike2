...
int main() 
{
     ...
     * MAX_SPIKES_PER_BUF (e.g. 500 points, a spike every 24 points, 4 
     * tetrodes per dsp = 500 / 24 * 4 spikes per buf max (~83)) */
    nspikepoints = 500;
            ...
            if (FD_ISSET(server_data[id].fd, &readfds)) {
			    ...
			    diobuf.timestamp, size);
                    ...
                    SendMessage(client_data[SPIKE_SAVE_DATA].fd, 
                            ...
                            DIO_BUF_STATIC_SIZE);
                    /* send the data on to the main program*/
     ...
     * sent to matlab and return the size of the buffer */
{
    ...
    for (j = 0; j < dptr->nchan; j++) {
        if (matlabinfo.contelect[dptr->electinfo[j].number]) {
            ...
            mptr->dspchan[mptr->nchan] = dptr->electinfo[j].dspchan[0];;
            mptr->electnum[mptr->nchan++] = dptr->electinfo[j].number;
        }
    }
